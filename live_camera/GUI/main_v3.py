...
from GUI.session_recorder import SessionRecorder  # STEP2
from GUI.stream_dialog import StreamDialog
from GUI.project_dialog import ProjectsDialog
...
LOG_POLL_MS=100
ALERT_HOLD_SECONDS=6.0  # ALERT-FIX: how long an alert stays on screen (banner, red border, label)
VIDEO_POLL_MS=33
RECORDING_FPS=20.0
...
class MainWindowV3(QMainWindow):
    ...
    voice_problem=Signal(str)  # ALERT-FIX: voice engine failures are shown to the operator
    recognizer_status=Signal(str)  # STEP3: warm-up result from the background thread
    ...
    def __init__(self):
        ...
        self._alert_until=0.0  # ALERT-FIX
        self._alert_text=""  # ALERT-FIX
        self._first_result_seen=False  # STEP3
        self._run_started_time=0.0  # STEP3
        self._last_wait_hint=0.0  # STEP3
        self._last_recognizer_error=None  # STEP3
        self.voice_alerts=VoiceAlerts(on_error=self.voice_problem.emit)  # ALERT-FIX
        self.voice_problem.connect(self._on_voice_problem)  # ALERT-FIX
        self.stream=UDPFrameStreamer()
        self.recorder=SessionRecorder(fps=RECORDING_FPS)  # STEP2: continuous full-session recording
        self.session_video_path=None  # STEP2
        self._last_archive_sig=None  # STEP2
        self._build_header()
        self._build_body()
        ...
        self.recognizer_status.connect(self._on_recognizer_status)  # STEP3: load the model now, not on the first frame
        threading.Thread(target=self._warmup_recognizer,daemon=True).start()
        ...
        self._bubble_timer=QTimer(self)  # ALERT-FIX: one timer, so an old timer can't hide a newer alert
        self._bubble_timer.setSingleShot(True)
        self._bubble_timer.timeout.connect(self.ai_bubble.hide)
    ...
    def _build_header(self):
        ...
        self.voice_toggle.setObjectName("voiceToggle")
        self.voice_toggle.setFixedSize(38,38)
        self.voice_toggle.setCursor(Qt.PointingHandCursor)
        self.voice_toggle.setToolTip("Pause voice alerts")
    ...
    def _load_experiment_json(self,path=None,saved_project=False):
        ...
        try:
            ...
            stored_json=project_dir/selected_path.name
            if selected_path.resolve()!=stored_json.resolve():
                stored_json.write_text(json.dumps(definition,indent=2),encoding="utf-8")
            self.current_experiment_path=stored_json
            ...
            self.validation_label.setText("Saved project loaded. Reference files are preserved.")
            if saved_project:
    ...
    def _start_loaded_experiment(self):
        ...
        try:
            ...
            self._first_result_seen=False  # STEP3
            self.qwen_recognizer.reset()  # STEP4: forget the previous run's match streak
            self._run_started_time=time.time()
            self._last_wait_hint=0.0
            self._start_session_recording()  # STEP2
            self._announce_step(self.live_controller.current_expected_id(),"Experiment started.")  # STEP2
    ...
    def _clear_to_initial_state(self):
        ...
        if self.live_controller is not None and self.live_controller.run_status=="RUNNING":  # STEP2
            self.live_controller.abort("experiment replaced or reset")
        self.live_controller=None
        self.experiment_finished=False
        self.experiment_def={}
        self.steps_def=[]
    ...
    def _abort_experiment(self):
        ...
        if answer!=QMessageBox.Yes:
            return
        try:  # STEP2: log the abort, save the archive, then return to the initial screen
            self.live_controller.abort("operator abort")
            self._poll_step_log()
        except Exception as e:
            print("Abort logging error:",e)
        self._clear_to_initial_state()

    def _reset_experiment_state(self):
        self._stop_session_recording()  # STEP2
        self.experiment_finished=False
        self._summary_shown=False
        self._analysis_busy=False
        self._last_analysis_time=0.0
    ...
    def _reference_dir(self):
        ...
        path=PROJECTS_DIR/self.current_experiment_path.stem/"reference_clips"
    ...
    def _update_video_frame(self):
        ...
        if self.live_controller is not None and not self.experiment_finished:  # STEP2
            self.recorder.write(frame)
        self.stream.maybe_send(frame,{"step":self._current_step_name,"status":self._overlay_status,"elapsed":self._current_timer_text,"ts":datetime.now().isoformat(timespec="seconds"),"alert":self._alert_text if self._alert_active() else ""})  # STEP2: alert text goes to ground too
    ...
    def _run_live_analysis(self,frame):
        try:
            ...
            print("Qwen error:",e)
            self.analysis_result.emit({"observed_action":"","matched_step_id":None,"confidence":0.0,"recognized":False,"error":f"{type(e).__name__}: {e}"})  # STEP3: never fail silently
        finally:
            with self._analysis_lock:
                self._analysis_busy=False
    ...
    def _handle_analysis_result(self,result):
        if self.live_controller is None or self.experiment_finished:
            return
        self._first_result_seen=True  # STEP3
        if result.get("error"):  # STEP3: show recognizer failures instead of hiding them
            msg=str(result["error"])
            if msg!=self._last_recognizer_error:
                self._last_recognizer_error=msg
                self._show_ai_bubble(f"Recognizer problem: {msg}","wrong",hold_ms=10000)
            if not self._alert_active():
                self.validation_label.setText(f"Validation: Recognizer problem - {msg}")
            self._log_observation(result,None)
            return
        self._last_recognizer_error=None
        ...
        self._log_observation(result,validation)  # STEP3
        if validation.get("status")=="UNCERTAIN":
            if not self._alert_active():  # ALERT-FIX
                self._overlay_status="pending"
                self.validation_label.setText(f"Validation: Waiting - {self._describe_uncertain(result)}")  # STEP3: say WHY nothing counted
            return
        if validation.get("status") in ("CORRECT","COMPLETE"):
            completed=validation.get("completed_step")
            self._alert_until=0.0  # ALERT-FIX: a correct step clears the alert
            if completed:
                self._save_event_clip(completed["step_id"],"CORRECT")
            self._overlay_status="done"
            completed=validation.get("completed_step",{})
            next_id=validation.get("current_expected_step_id")  # STEP2
            next_name=self.step_lookup.get(next_id,{}).get("name") if next_id is not None else None
            bubble=f"Nicely done: {completed.get('name','Step complete')}."
            if next_name:
                bubble+=f" Next: {next_name}."
            self._show_ai_bubble(bubble,"done")
            self.validation_label.setText(f"Validation: Step complete. Observed: {observed}.")
            QApplication.beep()
            if next_id is not None:  # STEP2: suggest the next step by voice
                self._announce_step(next_id,"Step complete.")
            else:
                self.voice_alerts.speak("Experiment complete. All steps done.")
        elif validation.get("status") in ("WRONG","OUT_OF_ORDER","UNEXPECTED"):
            self._handle_dynamic_error(validation)  # ALERT-FIX: alert FIRST, so a clip-saving problem can never block it
            try:
            ...
            expected_instruction=expected.get("instruction") or expected.get("name") or "the current step"
            spoken=f"Step already complete. Please perform {expected_instruction}."
            self._overlay_status="wrong"
            self._alert_until=time.time()+ALERT_HOLD_SECONDS
            self._alert_text=spoken
            self._show_ai_bubble(spoken,"wrong",hold_ms=int(ALERT_HOLD_SECONDS*1000))
            self.validation_label.setText(f"Validation: {spoken}")
            self.voice_alerts.speak(spoken,force=True)
            expected_id=expected.get("step_id")
            if expected_id is not None:
                self._auto_show_reference(expected_id)
        elif validation.get("status")=="PENDING_CONFIRMATION":
            if self._alert_active():  # ALERT-FIX
                return
            self._overlay_status="pending"
            self.validation_label.setText(f"Validation: confirming '{observed}' ({validation.get('confirmations',0)}/{validation.get('required_confirmations',3)}){self._latency_text(result)}...")

    def _describe_uncertain(self,result):  # STEP3
        observed=str(result.get("observed_action") or "").strip() or "nothing"
        conf=float(result.get("confidence") or 0.0)
        need=self.live_controller.min_confidence if self.live_controller else 0.6
        matched=result.get("matched_step_id")
        if matched is not None and conf<need:
            why=f"saw '{observed}' (step {matched}) but confidence {conf:.2f} is below {need:.2f}"
        elif not result.get("recognized"):
            why=f"saw '{observed}' - no clear action"
        elif matched is None:
            why=f"saw '{observed}' - it does not match a step"
        else:
            why=f"saw '{observed}'"
        return why+self._latency_text(result)

    @staticmethod
    def _latency_text(result):  # STEP3
        lat=result.get("latency_s")
        return f" [{lat:.1f}s per frame]" if lat else ""

    def _log_observation(self,result,validation):  # STEP3: every recognizer result, for tuning and debugging
        try:
            name=self.session_archive_path.name.replace("session_","observations_",1).replace(".json",".jsonl")
            record={
                "time":datetime.now().isoformat(timespec="milliseconds"),
                "observed":result.get("observed_action"),
                "matched_step_id":result.get("matched_step_id"),
                "confidence":result.get("confidence"),
                "recognized":result.get("recognized"),
                "latency_s":round(result["latency_s"],2) if result.get("latency_s") else None,
                "error":result.get("error"),
                "raw":result.get("raw"),
                "decision":validation.get("status") if validation else None,
                "verified":result.get("verified"),  # STEP4
                "vetoed_step_id":result.get("vetoed_step_id"),
                "verify_reason":result.get("verify_reason"),
            }
            with open(self.session_archive_path.with_name(name),"a",encoding="utf-8") as f:
                f.write(json.dumps(record)+"\n")
        except Exception as e:
            print("Observation log error:",e)

    def _warmup_recognizer(self):  # STEP3 (background thread)
        self.recognizer_status.emit(json.dumps(self.qwen_recognizer.warmup()))

    def _on_recognizer_status(self,payload):  # STEP3
        r=json.loads(payload)
        if r["ok"]:
            msg=f"Recognizer ready (model loaded in {r['seconds']:.1f}s)."
        else:
            msg=f"Recognizer unavailable: {r['error']}"
            self._last_recognizer_error=r["error"]
            self._show_ai_bubble(msg,"wrong",hold_ms=10000)
        print(msg)
        if self.live_controller is None:
    ...
    def _handle_dynamic_error(self,validation):
        ...
        expected_instruction=expected.get("instruction") or expected_name
        self._overlay_status="wrong"
        self._alert_until=time.time()+ALERT_HOLD_SECONDS
        if status=="OUT_OF_ORDER":
            skipped=validation.get("skipped_step_ids") or []
            skipped_names=[]
            for sid in skipped:
                step=self.step_lookup.get(sid,{})
                if step.get("name"):
                    skipped_names.append(step["name"])
            skipped_text=", ".join(skipped_names) if skipped_names else expected_name
            spoken=f"Step skipped. You performed {observed}. Please perform {skipped_text} first."
        elif status=="UNEXPECTED":
            spoken=f"Wrong step detected. Please perform {expected_instruction}."
        else:
            spoken=f"Wrong step detected. Please perform {expected_instruction}."
        self._alert_text=spoken
        self._show_ai_bubble(spoken,"wrong",hold_ms=int(ALERT_HOLD_SECONDS*1000))
    ...
    def _alert_active(self):  # ALERT-FIX
        return time.time()<self._alert_until

    def _on_voice_problem(self,message):  # ALERT-FIX
        text=f"Voice alerts unavailable - showing visual alerts only. ({message})"
        self.validation_label.setText(f"Validation: {text}")
        self._show_ai_bubble("Voice alerts unavailable. Check the console / run: python -m GUI.voice_alerts","wrong",hold_ms=8000)

    def _draw_alert_banner(self,frame,color):  # ALERT-FIX: burned into the frame, so it also appears in the streamed video
        h,w=frame.shape[:2]
        text=self._alert_text.upper()
        font=cv2.FONT_HERSHEY_SIMPLEX
        scale,thickness=0.8,2
        while scale>0.4 and cv2.getTextSize(text,font,scale,thickness)[0][0]>w-24:
            scale-=0.05
        ...
        cv2.putText(frame,text,(max(12,(w-tw)//2),bar_h-13),font,scale,(255,255,255),thickness,cv2.LINE_AA)

    def _draw_overlay(self,frame):
        alert=self._alert_active()  # ALERT-FIX
        color=hex_to_bgr(self.status_colors.get("wrong" if alert else self._overlay_status,self.theme["pending"]))
        h,w=frame.shape[:2]
        cv2.rectangle(frame,(0,0),(w-1,h-1),color,6)
        if alert and self._alert_text:
    ...
    def _poll_step_log(self):
        if self.live_controller is None:
            return
        if not self._first_result_seen and not self.experiment_finished and self._run_started_time:  # STEP3
            waited=time.time()-self._run_started_time
            if waited>8 and time.time()-self._last_wait_hint>4:
                self._last_wait_hint=time.time()
                self.validation_label.setText(f"Validation: Recognizer has not answered for {waited:.0f}s - model still loading or too slow (run diagnose_recognizer.py).")
    ...
    def _persist_archive(self,steps):
        ...
        ctl=self.live_controller  # STEP2
        history=list(getattr(ctl,"event_history",[])) if ctl else []
        run_status=getattr(ctl,"run_status",None)
        sig=(run_status,len(history),len(self.event_recordings),str(self.session_video_path),tuple((s["step_id"],s["status"]) for s in steps))
        if sig==self._last_archive_sig:  # nothing changed: don't rewrite the file 10x a second
            return
        self._last_archive_sig=sig
        session_logger.save_session(
            self.session_archive_path,
            self.session_started,
            enriched,
            self.event_recordings,
            extra={"run_status":run_status,"session_video":str(self.session_video_path) if self.session_video_path else None,"controller_events":history}
        )
    ...
    def _show_ai_bubble(self,text,status,hold_ms=4000):
        ...
        self._bubble_timer.start(hold_ms)  # ALERT-FIX
    ...
    def _start_session_recording(self):  # STEP2
        stamp=self.session_started.strftime("%Y-%m-%d_%H%M%S")
        path=RECORDINGS_DIR/f"session_{stamp}.mp4"
        if self.recorder.start(path):
            self.session_video_path=path
        else:
            self.session_video_path=None
            QMessageBox.warning(self,"Recording","Could not start full-session recording:\n\n"+str(self.recorder.last_error)+"\n\nThe experiment will still run and be logged.")

    def _stop_session_recording(self):  # STEP2
        if hasattr(self,"recorder") and self.recorder.active:
            self.recorder.stop()

    def _toggle_voice(self):
        try:
            if self.voice_alerts.is_paused():
                self.voice_alerts.resume()
                self.voice_toggle.setText("🔊")
                self.voice_toggle.setToolTip("Pause voice alerts")
            else:
                self.voice_alerts.pause()
                self.voice_toggle.setText("▶")
                self.voice_toggle.setToolTip("Resume voice alerts")
        except Exception as e:
            print("Voice toggle error:",e)

    def _announce_step(self,step_id,prefix=""):  # Spoken step transition
        step=self.step_lookup.get(step_id)
        if not step:
            return
        number=step.get("step_id",step_id)
        name=(step.get("name") or "").strip()
        instruction=(step.get("instruction") or name).strip()
        text=f"Now step {number}. Perform {instruction}."
        if prefix:
            text=f"{prefix} {text}"
        self.voice_alerts.speak(text,force=True)
    ...
    def closeEvent(self,event):
        if self.live_controller is not None and self.live_controller.run_status=="RUNNING":  # STEP2
            try:
                self.live_controller.abort("application closed")
                self._poll_step_log()
            except Exception as e:
                print("Close logging error:",e)
        self._stop_session_recording()  # STEP2
        if self.capture:
