...
class LogFilesDialog(QDialog):
    ...
    def _load_session(self,item):
        ...
        for row,event in enumerate(self._current_events):
            ...
            step=next((s for s in self._current_steps if s.get("step_id")==step_id),{})

            self.table.setItem(row,0,QTableWidgetItem(step.get("name",f"Step {step_id}")))
            self.table.setItem(row,1,QTableWidgetItem(event.get("status","")))
            self.table.setItem(row,2,QTableWidgetItem("-"))
            self.table.setItem(row,3,QTableWidgetItem("-"))
            self.table.setItem(row,4,QTableWidgetItem(event.get("video_path","")))
